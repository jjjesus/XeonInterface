...
This SATA interface is to the Linux boot image.  The K3 application will not access this image, but, an entry in \texttt{/etc/services.d} starts a service called \texttt{CSWMain} which is a shim program that loads the K3 application from the External SSD (Section \ref{section:sata2}).


\subsubsection{Setting up SATA (local boot flash)}
\label{section:setup_sata}
